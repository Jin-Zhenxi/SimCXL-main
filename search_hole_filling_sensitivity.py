#!/usr/bin/env python3
"""
Hole-filling sensitivity analysis around the fixed single-path baselines:
- ViT-Base-like @ rows_B = 48
- ViT-Large-like @ rows_B = 128

Fixed carry-over baseline for the second structural step:
...
CSV_FILE = os.path.join(OUTPUT_DIR, "hole_filling_sensitivity.csv")
BEST_FILE = os.path.join(OUTPUT_DIR, "hole_filling_sensitivity_best.csv")
...
PRESETS = [
    {
        ...
        "num_heads": 12,
        "rows_b": 48,
        "hole_fill_leads": [0, 8, 16, 24, 32, 48],
    },
    {
        "name": "ViT-Large-like",
        "seq_len": 257,
        ...
        "num_heads": 16,
        "rows_b": 128,
        "hole_fill_leads": [0, 16, 32, 48, 64, 96, 128],
    },
]
...
def row_key(preset_name, hole_fill_lead):
    return (preset_name, str(hole_fill_lead))
...
def run_one(preset, hole_fill_lead):
    run_tag = f"{preset['name'].replace(' ', '_').replace('/', '_')}__holelead_{hole_fill_lead}"

    rs.modify_c_file(preset)

    print(
        f"\n>>> Hole-filling sensitivity: {preset['name']} "
        f"rows_b={preset['rows_b']} hole_fill_lead={hole_fill_lead} <<<"
    )
    ...
    env["MATRIXFLOW_HOLE_FILL_LEAD_ROWS"] = str(hole_fill_lead)
    ...
    return {
        ...
        "carry_over_max_rows": 0,
        "carry_over_inherit_inflight": 0,
        "hole_fill_lead_rows": hole_fill_lead,
...
def main():
    ...
    print("🚀 Hole-filling Sensitivity Analysis")
    print("   固定 baseline: Base@48, Large@128")
    print("   固定 carry-over: max_rows=0, inherit_inflight=0")
    print("   机制旋钮: hole_fill_lead_rows")
    print("=" * 60)

    rows = load_existing_rows(CSV_FILE)
    completed = {
        row_key(row["Preset"], row["hole_fill_lead_rows"]) for row in rows
    }
    for preset in PRESETS:
        for lead in preset["hole_fill_leads"]:
            if row_key(preset["name"], lead) in completed:
                print(
                    f"[-] 跳过已完成点: {preset['name']} " f"hole_fill_lead={lead}"
                )
                continue
            row = run_one(preset, lead)
            rows.append(row)
            completed.add(row_key(preset["name"], lead))
            write_results(CSV_FILE, rows)
            print(
                f"[√] {preset['name']} lead={lead} "
