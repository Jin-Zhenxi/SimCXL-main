...
struct CXLMemTimingShim : public Packet::SenderState
{
    explicit CXLMemTimingShim(PacketPtr pkt)
        : addr(pkt->getAddr()), isWrite(pkt->isWrite())
    {
    }
...
CXLMemCtrl::write(PacketPtr pkt)
{
    ...
    if (bar == 0 && offset == 0x10000) {
        const auto hdmRanges = cxlRspPort.getAddrRanges();
        panic_if(hdmRanges.empty(),
                 "MatrixFlow doorbell fired without an HDM range.\n");
        const Addr defaultDescAddr = hdmRanges.front().start();
        const Addr descriptorAddr =
            pkt->getSize() >= sizeof(uint64_t) ?
                static_cast<Addr>(pkt->getLE<uint64_t>()) : defaultDescAddr;
        if (matrixEngine) {
            matrixEngine->startMatrixCompute(descriptorAddr);
        }
        DPRINTF(MatrixFlow,
                "🚀 [CXL.io Control Plane] BAR0 Doorbell Hit! "
                "Offset: %#lx, Triggering MatrixEngine with descriptor=%#llx.\n",
                offset, static_cast<unsigned long long>(descriptorAddr));
    }
...
CXLMemCtrl::CXLRequestPort::recvTimingResp(PacketPtr pkt)
{
    ...
    DPRINTF(MatrixFlow,
            "📥 [CXL.mem RESP] addr=%#lx cmd=%s needsResp=%d isResp=%d "
            "isWrite=%d senderState=%p\n",
            pkt->getAddr(), pkt->cmdString(), pkt->needsResponse(),
            pkt->isResponse(), pkt->isWrite(),
            static_cast<void *>(pkt->senderState));

    auto *shim = dynamic_cast<CXLMemTimingShim *>(pkt->senderState);
    panic_if(shim == nullptr,
             "CXL.mem timing response arrived without CXLMemTimingShim. "
             "addr=%#lx cmd=%s senderState=%p\n",
             pkt->getAddr(), pkt->cmdString(),
             static_cast<void *>(pkt->senderState));
    auto *popped = pkt->popSenderState();
    panic_if(popped != shim,
             "CXL.mem timing response popped unexpected shim. "
             "addr=%#lx cmd=%s popped=%p expected=%p\n",
             pkt->getAddr(), pkt->cmdString(),
             static_cast<void *>(popped), static_cast<void *>(shim));
    DPRINTF(MatrixFlow,
            "🔓 [CXL.mem RESP] restored addr=%#lx shimAddr=%#lx shimWrite=%d "
            "senderState=%p\n",
            pkt->getAddr(), shim->addr, shim->isWrite,
            static_cast<void *>(pkt->senderState));
    delete shim;
...
CXLMemCtrl::CXLResponsePort::recvTimingReq(PacketPtr pkt)
{
    ...
    DPRINTF(CXLMemCtrl, "recvTimingReq: %s addr 0x%x\n",
            pkt->cmdString(), paddr);
    DPRINTF(MatrixFlow,
            "📤 [CXL.mem REQ] addr=%#lx cmd=%s needsResp=%d isResp=%d "
            "isWrite=%d senderState=%p\n",
            paddr, pkt->cmdString(), pkt->needsResponse(),
            pkt->isResponse(), pkt->isWrite(),
            static_cast<void *>(pkt->senderState));
    ...
    if (memReqPort.reqQueueFull()) {
        ...
        if (!retryReq) {
            if (expects_response) {
                pkt->pushSenderState(new CXLMemTimingShim(pkt));
                DPRINTF(MatrixFlow,
                        "🛡️ [CXL.mem REQ] pushed shim for addr=%#lx cmd=%s "
                        "senderState=%p\n",
                        paddr, pkt->cmdString(),
                        static_cast<void *>(pkt->senderState));
            }
            DPRINTF(MatrixFlow,
                    "📦 [CXL.mem Data Plane] Transparent 64B forward. "
                    "addr=%#lx\n",
                    paddr);
            Tick receive_delay = pkt->headerDelay + pkt->payloadDelay;
            pkt->headerDelay = pkt->payloadDelay = 0;
