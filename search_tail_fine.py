#!/usr/bin/env python3
"""
Tail-focused fine search for the single-path CXL+DDR5 route.

This stage does not expand the search space. It only refines around:
- ViT-Base-like main peak and secondary-mode peak
- ViT-Large-like high-rows region
- a few kernel presets for generalization support

It also reuses already completed coarse-search rows from
my_outputData/small_tile_search_partial_snapshot.csv when possible.
"""
...
CSV_FILE = os.path.join(OUTPUT_DIR, "tail_fine_search.csv")
BEST_FILE = os.path.join(OUTPUT_DIR, "tail_fine_search_best.csv")
COARSE_SNAPSHOT = os.path.join(
    OUTPUT_DIR, "small_tile_search_partial_snapshot.csv"
)
...
TARGET_ROWS = {
    "ViT-Base-like": [40, 44, 48, 52, 56, 88, 96, 104, 112],
    "ViT-Large-like": [96, 104, 112, 120, 128, 136, 144, 160],
    "Kernel-S192": [40, 48, 56, 64],
    "Kernel-S224": [80, 96, 112],
    "Kernel-S320": [32, 40, 48, 56, 64],
}

LARGE_EXTENSION_ROWS = [176, 192]
...
def load_snapshot_rows():
    if not os.path.exists(COARSE_SNAPSHOT):
        return {}
    rows = list(csv.DictReader(open(COARSE_SNAPSHOT, newline="")))
    return {(r["Preset"], int(r["Prefetch Rows B"])): r for r in rows}


def load_existing_results():
    if not os.path.exists(CSV_FILE):
        return []
    return list(csv.DictReader(open(CSV_FILE, newline="")))
...
def append_or_replace(results, row):
    key = (row["Preset"], int(row["Prefetch Rows B"]))
    for idx, existing in enumerate(results):
        if (existing["Preset"], int(existing["Prefetch Rows B"])) == key:
            results[idx] = row
            return
    results.append(row)
...
def run_one(preset, rows_b):
    cfg = {
        "label": f"tailfine_compute_launch_{rows_b}",
...
def maybe_extend_large(rows_by_key):
    candidates = [96, 104, 112, 120, 128, 136, 144, 160]
    items = []
    for rb in candidates:
        row = rows_by_key.get(("ViT-Large-like", rb))
        if row:
            items.append(row)
    ...
    items.sort(key=score_row, reverse=True)
    best = items[0]
    if int(best["Prefetch Rows B"]) == 160:
        return LARGE_EXTENSION_ROWS
    return []
...
def main():
    ...
    snapshot = load_snapshot_rows()
    results = load_existing_results()
    for row in results:
        row["Prefetch Rows B"] = str(row["Prefetch Rows B"])

    print("🚀 Tail Fine Search")
    ...
    for preset in PRESETS:
        for rows_b in TARGET_ROWS[preset["name"]]:
            scheduled.append((preset, rows_b))

    done_keys = {(r["Preset"], int(r["Prefetch Rows B"])) for r in results}
    rows_by_key = {
        (r["Preset"], int(r["Prefetch Rows B"])): r for r in results
    }

    for preset, rows_b in scheduled:
        key = (preset["name"], rows_b)
        if key in done_keys:
            print(f"[-] 跳过已完成点: {preset['name']} rows_b={rows_b}")
            continue

        if key in snapshot:
            row = dict(snapshot[key])
            row["Source"] = "coarse_snapshot"
            append_or_replace(results, row)
            rows_by_key[key] = row
            done_keys.add(key)
            write_results(CSV_FILE, results)
            print(f"[√] 复用粗扫结果: {preset['name']} rows_b={rows_b}")
            continue
        ...
        write_results(CSV_FILE, results)
        print(
            f"[√] {preset['name']} rows_b={rows_b} "
            f"share={row['GEMM Compute Share (%)']:.4f}% "
            f"gemm={row['GEMM-Effective (GFLOPS)']:.3f} GFLOPS"
        )

    extra_large = maybe_extend_large(rows_by_key)
    if extra_large:
        print(f"\n[*] ViT-Large-like 在 rows_b=160 仍为最佳，追加延伸区间: {extra_large}")
        preset = next(p for p in PRESETS if p["name"] == "ViT-Large-like")
        for rows_b in extra_large:
            key = (preset["name"], rows_b)
            if key in done_keys:
                print(f"[-] 跳过已完成点: {preset['name']} rows_b={rows_b}")
                continue
    ...
    best_rows = pick_best_rows(results)
    write_results(BEST_FILE, best_rows)

    print("\n" + "=" * 60)
    print(f"✅ 精扫完成，结果写入: {CSV_FILE}")
    print(f"✅ 最佳点摘要写入: {BEST_FILE}")
