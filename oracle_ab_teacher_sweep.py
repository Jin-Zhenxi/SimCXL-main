#!/usr/bin/env python3
"""
A/B Oracle teacher sweep.

This script is intentionally separate from run_sweep.py:
- fixed multi-workload analysis entrypoint
- fixed mode set for Oracle teacher / online-rule extraction
- no dependence on run_sweep's current experiment narrowing

Oracle is the teacher.
The goal is to collect enough A/B-side behavior to derive online rules.
"""
...
CSV_FILE = os.path.join(OUTPUT_DIR, "ab_oracle_teacher_sweep.csv")
RAW_TXT_FILE = os.path.join(OUTPUT_DIR, "ab_oracle_teacher_sweep_raw.txt")
...
A_DISTANCES = ["immediate", "near", "far"]
A_REUSE = ["one", "multi"]
A_RECURRENCE = ["first", "repeat"]
...
def completed_case_keys(rows):
    ...
    for row in rows:
        if row.get("Preset") and row.get("Prefetch Label"):
            out.add((row["Preset"], row["Prefetch Label"]))
    return out
...
def ensure_gem5_binary():
    ...
    if not rebuild:
        ...
        return
    print("[*] 检测到 Oracle A/B 相关源码更新，正在通过 build_p2p.sh 重建 gem5.opt ...")
    if BUILD_P2P.exists():
        subprocess.run(
            "./build_p2p.sh",
            shell=True,
...
def active_configs():
    ...
    for label in MODE_LABELS:
        cfg = next(deepcopy(item) for item in rs.PREFETCH_CONFIGS if item["label"] == label)
        out.append(cfg)
    return out
...
def parse_vector2d(stats_file, base_name, sources, distances, prefix):
    values = {}
    ...
    for src in sources:
        for dist in distances:
            key = f"{prefix}_{src}_{dist}"
            pattern = rf"matrix_engine\.{base_name}_{src}::{dist}\s+([0-9.]+)"
            matches = list(re.finditer(pattern, text))
            values[key] = float(matches[-1].group(1)) if matches else 0.0
    return values


def parse_vector(stats_file, base_name, buckets, prefix):
    values = {}
    if not os.path.exists(stats_file):
        return values
    text = Path(stats_file).read_text(encoding="utf-8", errors="ignore")
    for bucket in buckets:
        ...
        pattern = rf"matrix_engine\.{base_name}::{bucket}\s+([0-9.]+)"
        matches = list(re.finditer(pattern, text))
        values[key] = float(matches[-1].group(1)) if matches else 0.0
    return values
...
def build_row_from_outputs(preset, cfg, rows_b, m5out_dir, log_file):
    ...
    row = {
        ...
        "next_output_prefetch_issue_count": prefetch["next_output_prefetch_issue_count"],
        "next_output_rows_ready_at_boundary": prefetch["next_output_rows_ready_at_boundary"],
        "next_output_consumed_before_fallback_rows": prefetch["next_output_consumed_before_fallback_rows"],
        "next_output_prefetch_rows_issued": prefetch["next_output_prefetch_rows_issued"],
        "future_claim_set_count": prefetch["future_claim_set_count"],
        "future_claim_blocked_normal_fetch_count": prefetch["future_claim_blocked_normal_fetch_count"],
        "future_claim_consumed_success_count": prefetch["future_claim_consumed_success_count"],
        "vip_rescue_insert_count": prefetch["vip_rescue_insert_count"],
        "vip_rescue_hit_count": prefetch["vip_rescue_hit_count"],
        "vip_rescue_prevented_fallback_count": prefetch["vip_rescue_prevented_fallback_count"],
        "vip_rescue_reused_count": prefetch["vip_rescue_reused_count"],
        "vip_rescue_a_insert_count": prefetch.get("vip_rescue_a_insert_count", 0),
        "vip_rescue_a_hit_count": prefetch.get("vip_rescue_a_hit_count", 0),
        "vip_rescue_a_prevented_remote_fetch_count": prefetch.get(
            "vip_rescue_a_prevented_remote_fetch_count", 0
        ),
        "vip_rescue_a_reused_count": prefetch.get("vip_rescue_a_reused_count", 0),
    }
    row.update(parse_vector2d(stats_file, "oracleLocalRows", B_SOURCES, B_DISTANCES, "oracle_b_local"))
    row.update(parse_vector2d(stats_file, "oracleFallbackRows", B_SOURCES, B_DISTANCES, "oracle_b_fallback"))
    row.update(
        parse_vector2d(
            stats_file, "oracleLocalReuseWeight", B_SOURCES, B_DISTANCES, "oracle_b_local_reuse_weight"
        )
    )
    row.update(
        parse_vector2d(
            ...
            B_SOURCES,
            B_DISTANCES,
            "oracle_b_fallback_reuse_weight",
        )
    )
    row.update(parse_vector(stats_file, "oracleALocalByDistance", A_DISTANCES, "oracle_a_local_distance"))
    row.update(parse_vector(stats_file, "oracleARemoteByDistance", A_DISTANCES, "oracle_a_remote_distance"))
    row.update(parse_vector(stats_file, "oracleALocalByReuse", A_REUSE, "oracle_a_local_reuse"))
    row.update(parse_vector(stats_file, "oracleARemoteByReuse", A_REUSE, "oracle_a_remote_reuse"))
    row.update(
        parse_vector(
            stats_file,
            "oracleALocalByRecurrence",
            A_RECURRENCE,
            "oracle_a_local_recurrence",
        )
    )
    row.update(
        parse_vector(
            stats_file,
            "oracleARemoteByRecurrence",
            A_RECURRENCE,
            "oracle_a_remote_recurrence",
        )
    )
    return row


def sweep_row_has_signal(row):
    return any(key.startswith("oracle_") and float(value) != 0.0 for key, value in row.items())


def run_one(preset, cfg, rows_b):
    preset_tag = preset["name"].replace(" ", "_").replace("/", "_")
    run_tag = f"{preset_tag}__ab_oracle__{cfg['label']}"

    rs.modify_c_file(preset)
    print(f"\n>>> A/B Oracle sweep: {preset['name']} / {cfg['label']} (rows_b={rows_b}) <<<")
    ...
    env["MATRIXFLOW_CARRY_OVER_MAX_ROWS"] = str(cfg.get("carry_over_max_rows", 0))
    env["MATRIXFLOW_CARRY_OVER_INHERIT_INFLIGHT"] = str(cfg.get("carry_over_inherit_inflight", 0))
    env["MATRIXFLOW_HOLE_FILL_LEAD_ROWS"] = str(cfg.get("hole_fill_lead_rows", 0))
    env["MATRIXFLOW_WRITEC_OVERLAP_B_ISSUE_BUDGET_ROWS"] = str(
        cfg.get("writec_overlap_b_issue_budget_rows", 0)
    )
    env["MATRIXFLOW_VIP_B_ROWS_CAPACITY"] = str(cfg.get("vip_b_rows_capacity", 0))
    env["MATRIXFLOW_AB_SCHEDULER_MODE"] = cfg.get("ab_scheduler_mode", "baseline")
    env["MATRIXFLOW_AB_A_MIN_CREDIT_ROWS"] = str(cfg.get("ab_a_min_credit_rows", 16))
    ...
    env["MATRIXFLOW_AB_W_FALLBACK_RISK"] = str(cfg.get("ab_weight_fallback_risk", 2))
    env["MATRIXFLOW_AB_MIN_LAUNCH_ROWS_A"] = str(cfg.get("ab_min_launch_rows_a", 0))
    env["MATRIXFLOW_AB_MIN_LAUNCH_ROWS_B"] = str(cfg.get("ab_min_launch_rows_b", 0))
    ...
    for attempt in range(1, MAX_INVALID_RUN_RETRIES + 1):
        ...
        if rc.returncode != 0:
            raise SystemExit(f"gem5 仿真失败，返回码={rc.returncode}。请检查日志: {log_file}")
...
def main():
    ...
    stale = existing_results_are_stale()
    if FORCE_RERUN:
        print("[*] FORCE_RERUN=1，本轮忽略已有 A/B Oracle sweep 结果。")
    elif stale:
        print("[*] 检测到已有 A/B Oracle sweep 结果早于当前 gem5.opt，将重新全量跑。")

    results = [] if (FORCE_RERUN or stale) else load_existing_results()
    completed = set() if (FORCE_RERUN or stale) else completed_case_keys(results)
    by_key = {(row["Preset"], row["Prefetch Label"]): row for row in results}

    print("🚀 A/B Oracle Teacher Sweep")
    print(
        "   工作集: "
        + ", ".join(f"{p['name']}[S={p['seq_len']},H={p['hidden_dim']}]" for p in ANALYSIS_PRESETS)
    )
    print("   模式: " + ", ".join(cfg["label"] for cfg in configs))
    print("   目标: Oracle=老师；在线规则另行提炼")
    print("=" * 60)
    ...
    for cfg in configs:
        for preset in ANALYSIS_PRESETS:
            ...
            if case_key in completed:
                ...
                if existing and sweep_row_has_signal(existing):
                    print(f"[-] 跳过已完成点: {preset['name']} / {cfg['label']}")
                    continue
            row = run_one(preset, cfg, rows_b)
            replaced = False
            for i, old in enumerate(results):
                if old.get("Preset") == preset["name"] and old.get("Prefetch Label") == cfg["label"]:
                    results[i] = row
                    replaced = True
                    break
            if not replaced:
    ...
    print("\n" + "=" * 60)
    print(f"✅ A/B Oracle sweep 完成，结果写入: {CSV_FILE}")
