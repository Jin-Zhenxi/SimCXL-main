#!/bin/bash
# 将 trigger_gemm 注入 parsec.img，使 Guest 无需网络即可运行
# 用法: ./inject_trigger_gemm.sh [--no-compile] [SIZE]
#   --no-compile: 跳过编译，使用已有的 trigger_gemm（供 run_sweep.py 使用）
...
# 1. 编译 trigger_gemm（除非 --no-compile，需 libm5.a：cd util/m5 && scons build/x86）
...
if [ -z "\$TRIGGER" ]; then
    ...
    rm -f "\$TMP_TRIGGER"
    if wget -T 3 -t 1 -q http://10.0.2.2:8000/trigger_gemm -O "\$TMP_TRIGGER" 2>/dev/null || \
       wget -T 3 -t 1 http://10.0.2.2:8000/trigger_gemm -O "\$TMP_TRIGGER" 2>/dev/null; then
...
sudo cp trigger_gemm "$MNT/root/trigger_gemm"
sudo cp trigger_gemm "$MNT/home/test_code/trigger_gemm"
sudo cp script.sh "$MNT/root/script.sh"
sudo cp script.sh "$MNT/home/test_code/script.sh"
sudo chmod +x "$MNT/root/trigger_gemm" "$MNT/home/test_code/trigger_gemm"
sudo chmod +x "$MNT/root/script.sh" "$MNT/home/test_code/script.sh"
echo "已注入到 /root/trigger_gemm、/home/test_code/trigger_gemm 以及 script.sh"
echo "=== 完成 ==="
